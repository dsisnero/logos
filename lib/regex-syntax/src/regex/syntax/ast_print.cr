module Regex::Syntax::AST
  class Printer
    ...
    private def write_node(node : ClassUnicode, io : IO) : Nil
      io << (node.negated? ? "\\P" : "\\p")
      if node.name.size == 1 && unicode_class_can_use_short_form?(node)
        io << node.name
      else
        io << "{" << node.name << "}"
      end
    ...
    private def unicode_class_can_use_short_form?(node : ClassUnicode) : Bool
      (node.span.end.offset - node.span.start.offset) <= 3
    end
