module Regex::Syntax::Hir
  alias HirKind = Node
  alias ClassBytes = CharClass
  alias ClassUnicode = UnicodeClass
  ...
  class Class
    ...
    getter kind : Kind
    getter unicode : UnicodeClass?
    getter bytes : CharClass?

    def initialize(@unicode : UnicodeClass)
      @kind = Kind::Unicode
      @bytes = nil
    end

    def initialize(@bytes : CharClass)
      @kind = Kind::Bytes
      @unicode = nil
    end

    def self.unicode(value : UnicodeClass) : self
      new(value)
    end

    def self.bytes(value : CharClass) : self
      new(value)
    end
  ...
  struct ClassBytesRange
    ...
    def initialize(start : UInt8, finish : UInt8)
      if start <= finish
        @start = start
        @end = finish
      else
    ...
    def to_range : Range(UInt8, UInt8)
      @start..@end
    end
  end

  class ClassBytesIter
    include Iterator(ClassBytesRange)

    def initialize(@ranges : Array(Range(UInt8, UInt8)), @index : Int32 = 0)
    end

    def next
      return stop if @index >= @ranges.size

      range = @ranges[@index]
      @index += 1
      ClassBytesRange.new(range.begin, range.end)
    end
  end
  ...
  struct ClassUnicodeRange
    ...
    def initialize(start : UInt32, finish : UInt32)
      if start <= finish
        @start = start
        @end = finish
      else
    ...
    def to_range : Range(UInt32, UInt32)
      @start..@end
    end
  end

  class ClassUnicodeIter
    include Iterator(ClassUnicodeRange)

    def initialize(@ranges : Array(Range(UInt32, UInt32)), @index : Int32 = 0)
    end
    ...
    def next
      ...
      range = @ranges[@index]
      @index += 1
      ClassUnicodeRange.new(range.begin, range.end)
    end
  end
  ...
  class CharClass < Node
    ...
    def initialize(ranges : Enumerable(ClassBytesRange))
      @intervals = IntervalOps.canonicalize(ranges.map(&.to_range).to_a)
      @negated = false
    end
    ...
    def push(range : ClassBytesRange) : self
      push(range.to_range)
    end
    ...
    def iter : ClassBytesIter
      ClassBytesIter.new(@intervals)
    end
  ...
  class UnicodeClass < Node
    ...
    def initialize(ranges : Enumerable(ClassUnicodeRange))
      @intervals = IntervalOps.canonicalize(ranges.map(&.to_range).to_a)
      @negated = false
    end
    ...
    def push(range : ClassUnicodeRange) : self
      push(range.to_range)
    end
    ...
    def iter : ClassUnicodeIter
      ClassUnicodeIter.new(@intervals)
    end
  ...
  class LookSet
    ...
    def set_insert(kind : Look::Kind) : self
      insert!(kind)
    end
    ...
    # ameba:disable Naming/AccessorMethodName
    def set_remove(kind : Look::Kind) : self
      remove!(kind)
    end
    ...
    # ameba:disable Naming/AccessorMethodName
    def set_subtract(other : LookSet) : self
      subtract!(other)
    end
    ...
    # ameba:disable Naming/AccessorMethodName
    def set_union(other : LookSet) : self
      union!(other)
    end
    ...
    # ameba:disable Naming/AccessorMethodName
    def set_intersect(other : LookSet) : self
      intersect!(other)
    end
  ...
  class LookSetIter
    include Iterator(Look::Kind)

    def initialize(look_set : LookSet, @index : Int32 = 0)
      @kinds = look_set.to_a
    end

    def next
      return stop if @index >= @kinds.size
  ...
  class Hir < Node
    ...
    def self.literal(literal : Literal) : Hir
      literal(literal.bytes)
    end

    def self.concat(children : Array(Hir)) : Hir
      concat(children.map(&.node))
    end

    def self.alternation(children : Array(Hir)) : Hir
      alternation(children.map(&.node))
    end
    ...
    def self.alternation(children : Array(Node)) : Hir
      ...
      if lifted = lift_common_prefix(flattened)
        return lifted
      end
      case flattened.size
      when 0
    ...
    # ameba:disable Naming/PredicateName
    def is_alternation_literal : Bool
      alternation_literal?
    end
    ...
    private def self.lift_common_prefix(children : Array(Node)) : Hir?
      return nil if children.size < 2

      sequences = children.map do |child|
        case child
        when Concat
          child.children
        else
      ...
      prefix_len = 0
      loop do
        break if sequences.any? { |sequence| prefix_len >= sequence.size }
        candidate = sequences.first[prefix_len]
        break unless sequences.all? { |sequence| nodes_equal?(sequence[prefix_len], candidate) }
        prefix_len += 1
      end

      return nil if prefix_len == 0

      prefix = sequences.first[0, prefix_len]
      suffixes = sequences.map do |sequence|
        remainder = sequence[prefix_len..]
        remainder ? remainder : ([] of Node)
      end
      ...
      suffix_nodes = suffixes.map do |suffix|
        concat(suffix).node
      end
      concat(prefix + [alternation(suffix_nodes).node])
    end

    private def self.nodes_equal?(left : Node, right : Node) : Bool
      if left.is_a?(Empty) && right.is_a?(Empty)
        true
      elsif left.is_a?(Literal) && right.is_a?(Literal)
        l = left.as(Literal)
        r = right.as(Literal)
        l.bytes == r.bytes
      elsif left.is_a?(DotNode) && right.is_a?(DotNode)
        l = left.as(DotNode)
        r = right.as(DotNode)
        l.kind == r.kind
      elsif left.is_a?(Look) && right.is_a?(Look)
        l = left.as(Look)
        r = right.as(Look)
        l.kind == r.kind
      elsif left.is_a?(CharClass) && right.is_a?(CharClass)
        l = left.as(CharClass)
        r = right.as(CharClass)
        l.negated? == r.negated? && l.intervals == r.intervals
      elsif left.is_a?(UnicodeClass) && right.is_a?(UnicodeClass)
        l = left.as(UnicodeClass)
        r = right.as(UnicodeClass)
        l.negated? == r.negated? && l.intervals == r.intervals
      elsif left.is_a?(Repetition) && right.is_a?(Repetition)
        l = left.as(Repetition)
        r = right.as(Repetition)
        l.min == r.min &&
          l.max == r.max &&
          l.greedy? == r.greedy? &&
          nodes_equal?(l.sub, r.sub)
      elsif left.is_a?(Capture) && right.is_a?(Capture)
        l = left.as(Capture)
        r = right.as(Capture)
        l.index == r.index &&
          l.name == r.name &&
        ...
        l = left.as(Concat)
        r = right.as(Concat)
        l.children.size == r.children.size &&
          l.children.zip(r.children).all? { |lhs, rhs| nodes_equal?(lhs, rhs) }
      elsif left.is_a?(Alternation) && right.is_a?(Alternation)
        l = left.as(Alternation)
        r = right.as(Alternation)
        l.children.size == r.children.size &&
          l.children.zip(r.children).all? { |lhs, rhs| nodes_equal?(lhs, rhs) }
      else
        false
