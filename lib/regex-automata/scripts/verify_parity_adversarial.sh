...
# Exclude lib/ directories (dependencies) and only check project code
if [[ -d "${ROOT_DIR}/spec" ]]; then
  # Find project directories to check (exclude lib/)
  check_dirs=()
  [[ -d "${ROOT_DIR}/spec" ]] && check_dirs+=("${ROOT_DIR}/spec")
  [[ -d "${ROOT_DIR}/src" ]] && check_dirs+=("${ROOT_DIR}/src")

  # Filter out lib/ directories
  filtered_dirs=()
  ...
  if [[ ${#filtered_dirs[@]} -gt 0 ]]; then
    if rg --pcre2 -n "^\s*pending(?!\s*(=|<<|\+=|-=|\*=|\/=))(\s|$)|^\s*xit\(|^\s*xdescribe\(|^\s*xcontext\(" "${filtered_dirs[@]}" >/dev/null 2>&1; then
      echo "Found placeholder specs in project code. Resolve before parity signoff." >&2
      echo "Note: Placeholder tests should be marked as 'skipped' in the port inventory with explanation." >&2
      exit 1
    fi
  fi
fi
